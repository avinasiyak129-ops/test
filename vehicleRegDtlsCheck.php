...
// --- 5. UPDATE ACTION (Always via PK for safety) ---
if ($mode === 'edit' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_record'])) {
    ...
    $execute_params['target_pk'] = $pk;
    $sql = "UPDATE rasvehicleregdtls_tbl SET " . implode(', ', $setParts) . " WHERE rasvehicleregdtls_pk = :target_pk";
...
if ($mode === 'edit') {
    ...
    if ($search !== '') {
        // Clean search input: remove hyphens
        $cleanSearch = str_replace('-', '', $search);
        // SQL: Remove hyphens from DB column during comparison
        $query .= " WHERE REPLACE(rvrd_vechicleregno, '-', '') = :search";
        $stmt = $pdo->prepare($query);
        $stmt->execute(['search' => $cleanSearch]);
    } else {
        // Broad view if no search is active
        $stmt = $pdo->query($query . " ORDER BY rasvehicleregdtls_pk DESC LIMIT 50");
    }
...
<head>
    ...
    <style>
        ...
        label { font-size: 11px; font-weight: bold; color: #606770; text-transform: uppercase; }
        input { padding: 10px; border: 1px solid #ccd0d5; border-radius: 6px; }
        .btn { padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; font-weight: 600; }
...
<div class="container">
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($results)): ?>
                    <tr><td colspan="5" style="text-align:center; padding:50px;">No records match your search criteria.</td></tr>
                <?php else: ?>
                    <?php foreach ($results as $row): ?>
                    <tr onclick="window.location='?id=<?= $row['rasvehicleregdtls_pk'] ?>'">
                        <td>#<?= $row['rasvehicleregdtls_pk'] ?></td>
                        <td style="color:#1877f2; font-weight:bold; font-size: 15px;"><?= htmlspecialchars($row['rvrd_vechicleregno']) ?></td>
    ...
    <?php else: ?>
        ...
        <form method="post" onsubmit="return confirm('Commit changes to live database?');">
            <div class="form-grid">
